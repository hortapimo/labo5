...
def analizar_espectro(archivo_csv, carpeta_destino):
    ...
    #label_grande = r"$\lambda_{medido}$ = " f"({longitud_pico:.1f} \u00b1 {incerteza:.1f}) nm\n"
    #plt.plot(longitud_pico, amplitud_pico, "rx", markersize=12, markeredgewidth=2, label=label_grande)
    ...
    label_grande = r"$\lambda_{medido}$ = " f"({longitud_pico:.1f} \u00b1 {ancho_banda:.2f}) nm"
    plt.axvline(x=longitud_pico, color="red", linestyle="--", linewidth=2, label=label_grande)
  ...
  #  plt.plot([], [], ' ', label=f"Δλ (desplazamiento instrumental) = ({offset:.1f} \u00b1 {incerteza:.1f}) nm\n")
    ...
    plt.hlines(y=mitad_intensidad, xmin=lambda_izq, xmax=lambda_der, color="green", linestyle="--", linewidth=2, label="Ancho de banda (incerteza)")
...
def analizar_espectro(archivo_csv, carpeta_destino):
    ...
    label_grande = r"$\lambda_{medido}$ = " f"({longitud_pico:.2f} \u00b1 {ancho_banda:.2f}) nm"
    plt.axvline(x=longitud_pico, color="red", linestyle="--", linewidth=2, label=label_grande)
    
    if not np.isnan(lambda_pedido):
        plt.axvline(x=lambda_pedido, color="orange", linestyle="-.", linewidth=2, label=r"$\lambda_{pedido}$ = " f"{lambda_pedido:.2f} nm")
                      
    # --- MODIFICACIÓN: Ancho de banda como error en el eje X ---
    # Lo graficamos como un error en X centrado en la longitud del pico medido.
    # El valor "xerr" es la mitad del ancho de banda para que el total abarque todo el ancho.
    #error_x = ancho_banda / 2.0
    #plt.errorbar(x=longitud_pico, y=amplitud_pico, xerr=error_x, fmt='o', color='green', 
     #            ecolor='green', elinewidth=2, capsize=5, capthick=2, 
      #           label=f"Ancho de banda (incerteza): {ancho_banda:.2f} nm")
